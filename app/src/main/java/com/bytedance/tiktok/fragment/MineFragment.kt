...
import com.bytedance.tiktok.databinding.FragmentMineHomeBinding
 ...
 */
class MineFragment : BaseBindingFragment<FragmentMineHomeBinding>({FragmentMineHomeBinding.inflate(it)})
    ...
    fun initMineView() {
        binding.homeHeader.tvAddfocus.visibility = View.INVISIBLE
        binding.homeHeader.rlDropdown.visibility = View.INVISIBLE

        //作者主页数据
        RxBus.getDefault().post(CurUserBean(DataCreate.datas[0]?.userBean!!))
    }
    fun init() {
